...
class Api:
    ...
    def authenticate(self, email=None, password=None, source=None):
        ...
        # Auth parameters
        Api.yt_service.email = email if email else settings.YOUTUBE_AUTH_EMAIL.encode('utf-8')
        Api.yt_service.password = password if password else settings.YOUTUBE_AUTH_PASSWORD.encode('utf-8')
        Api.yt_service.source = source if source else settings.YOUTUBE_CLIENT_ID.encode('utf-8')
        try:
