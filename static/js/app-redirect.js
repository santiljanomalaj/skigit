...
(function () {
        ...
        redirect: function (options) {
            ...
            */
            ...
            if (hasIos && /iP(hone|ad|od)/.test(navigator.userAgent)) {
                ...
                // check is Pinterest app browser
                if(/Pinterest/.test(navigator.userAgent)){
                    window.location = this.clearUrl(intent.scheme + intent.host);
                    setTimeout(function(){
                        window.location = encodeURIComponent(intent.package);
                    }, 100);
                } else {
                    // other brower
                    var intentUrl = 'intent://' + intent.host + '#Intent;' +
                        ...
                        'end';
                    // var intentUrl = intent.scheme + '://' + intent.host;
                    intentUrl = this.clearUrl(intentUrl);
    ...
    if (typeof define === 'function') {
        // AMD
        define('appRedirect', [], function () { return AppRedirect; });
